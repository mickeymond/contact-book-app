...
import { Layout, Form, Input, Icon, Button, Row, Col, Spin, Alert } from 'antd/es';
        ...
        <Col sm={12} offset={6}>
          <Form onSubmit={e => {
            ...
            editContact({
              variables: {
                id: id,
                first_name: (firstNameEl.current as unknown as Input).state.value,
                last_name: (lastNameEl.current as unknown as Input).state.value,
                twitter_username: (twitterUsernameEl.current as unknown as Input).state.value
              }
            })
            .catch(console.log)
            ...
            <Form.Item>
              <Button type="primary" htmlType="submit" className="login-form-button">Edit Contact</Button>
            </Form.Item>
          </Form>
        </Col>
      </Row>
